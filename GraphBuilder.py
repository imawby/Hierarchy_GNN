...
def GraphBuilder(eventDict, modeDict) :
    ...
    for iSourceParticle in range(eventDict["nParticles"]) :
        ...
        # If we don't know the truth match
        if (eventDict["trueTrackID"][iSourceParticle] < 0) :
            continue
            
        # If it isn't the neutrino, and we don't know who the parent is
        if ((eventDict["isNeutrinoPDG"][iSourceParticle] == 0) and (eventDict["trueVisibleParentTrackID"][iSourceParticle] < 0)) : 
            continue
    ...
    print('eventDict - isNu:', eventDict["isNeutrinoPDG"])
    print('pfpIndex:', pfpIndex)
    print('trueTrackID:', eventDict["trueTrackID"])
    print('true parent:', eventDict["trueVisibleParentTrackID"])
    
    # Create our NU-PARTICLE edges and POS-NEG graphs
    # in this setup iSourceParticle is never the neutrino...
    for iSourceParticle in range(nGraphNodes) :
        ...
        for iTargetParticle in range((iSourceParticle + 1), nGraphNodes) :
            ...
            print('iSourceParticle:', iSourceParticle)
            print('iTargetParticle:', iTargetParticle)
            
            print('sourcePFPIndex:', sourcePFPIndex)
            print('targetPFPIndex:', targetPFPIndex)
            
            print('is source nu?', eventDict["isNeutrinoPDG"][sourcePFPIndex])
            print('is target nu?', eventDict["isNeutrinoPDG"][targetPFPIndex])
            
            print('source track ID:', eventDict["trueTrackID"][sourcePFPIndex])
            print('target parent track ID:', eventDict["trueVisibleParentTrackID"][targetPFPIndex])
            
            
            print('iParent:', iParent)
            print('iChild:', iChild)
...
#################################################################################################################################################
# Inference graph Builder Function
#################################################################################################################################################
def InferenceGraphBuilder(eventDict, modeDict) :
    ...
    # Now start to build graph, first find nodes
    nGraphNodes = 0
    ...
    # Bail if we have only have neutrino graph node   
    if (nGraphNodes == 1) :
        event_node_features = torch.tensor(event_node_features, dtype=torch.float)
        event_edge_index_FC = torch.tensor([target_index_FC, source_index_FC], dtype=torch.long)
        edge_attr_FC = torch.tensor(edge_attr_FC, dtype=torch.float)
        
        return Data(x=event_node_features, edge_index=event_edge_index_FC, y=event_node_truth, edge_attr=edge_attr_FC), np.empty(0)
    ...
    for iSourceParticle in range(nGraphNodes) :
        ...
        for iTargetParticle in range((iSourceParticle + 1), nGraphNodes) :
            ...
            # get edge_attr (1.0 is a 'yes i am primary')
            thisEdgeAttr = [displacement, dca, nuVertexEnergyAsymmetry, nuVertexEnergyWeightedMeanRadialDistance, 1.0]
            
            # Fill message passing graph info
            if isNeutrinoLink :
                
                iNu = iSourceParticle if (eventDict["isNeutrinoPDG"][sourcePFPIndex] == 1) else iTargetParticle
                iChild = iTargetParticle if (eventDict["isNeutrinoPDG"][sourcePFPIndex] == 1) else iSourceParticle
                
                nuIndex = sourcePFPIndex if (eventDict["isNeutrinoPDG"][sourcePFPIndex] == 1) else targetPFPIndex
                childIndex = targetPFPIndex if (eventDict["isNeutrinoPDG"][sourcePFPIndex] == 1) else sourcePFPIndex
    
                # Add one way
                source_index_FC.append(iNu)
                target_index_FC.append(iChild)
                edge_attr_FC.append(thisEdgeAttr)
            else :
                # Calculate opening angle - do need to worry about normalisation
                sourceDirection = np.array(showerDir_notNorm[sourcePFPIndex])
                targetDirection = np.array(showerDir_notNorm[targetPFPIndex])
    ...
    for index in range(math.floor(len(particle_particle_edges) * modeDict["EDGE_FRACTION"])) :
        ...
        source_index_FC.append(particle_particle_edges['source'][index])
        target_index_FC.append(particle_particle_edges['target'][index]) 
        edge_attr_FC.append([-2.0, -2.0, -2.0, -2.0, 0.0])
        # Other way 
        source_index_FC.append(particle_particle_edges['target'][index])
        target_index_FC.append(particle_particle_edges['source'][index])        
        edge_attr_FC.append([-2.0, -2.0, -2.0, -2.0, 0.0])
    ...
    # turn into data
    data_FC = Data(x=event_node_features, edge_index=event_edge_index_FC, edge_attr=edge_attr_FC)
                  
    return data_FC, np.array(pfpIndex)




#################################################################################################################################################
# Inference graph Builder Function - heterogenous-ish??
#################################################################################################################################################
def EdgeAttributeGraphBuilder(eventDict, modeDict) :
    ...
    if (nGraphNodes == 1) :
        ...
        #event_edge_index_neg = torch.tensor([source_index_neg, target_index_neg], dtype=torch.long)
        #event_edge_index_FC = torch.tensor([source_index_FC, target_index_FC], dtype=torch.long)
        event_edge_index_pos = torch.tensor([target_index_pos, source_index_pos], dtype=torch.long)
        event_edge_index_neg = torch.tensor([target_index_neg, source_index_neg], dtype=torch.long)
        event_edge_index_FC = torch.tensor([target_index_FC, source_index_FC], dtype=torch.long)
        edge_attr_pos = torch.tensor(edge_attr_pos, dtype=torch.float)
        edge_attr_neg = torch.tensor(edge_attr_neg, dtype=torch.float)
        edge_attr_FC = torch.tensor(edge_attr_FC, dtype=torch.float)
        
        return Data(x=event_node_features, edge_index=event_edge_index_pos, y=event_node_truth, edge_attr=edge_attr_pos), \
        Data(x=event_node_features, edge_index=event_edge_index_neg, y=event_node_truth, edge_attr=edge_attr_neg), \
        Data(x=event_node_features, edge_index=event_edge_index_FC, y=event_node_truth, edge_attr=edge_attr_FC), \
        np.empty(0)
    ...
    for iSourceParticle in range(nGraphNodes) :
        ...
        for iTargetParticle in range((iSourceParticle + 1), nGraphNodes) :
            ...
            parentIndex = sourcePFPIndex if (eventDict["trueTrackID"][sourcePFPIndex] == eventDict["trueVisibleParentTrackID"][targetPFPIndex]) else targetPFPIndex
            childIndex = targetPFPIndex if (eventDict["trueTrackID"][sourcePFPIndex] == eventDict["trueVisibleParentTrackID"][targetPFPIndex]) else sourcePFPIndex
      
            # get edge_attr (1.0 is a 'yes i am primary')
            thisEdgeAttr = [displacement, dca, nuVertexEnergyAsymmetry, nuVertexEnergyWeightedMeanRadialDistance, 1.0]
            #[0.0 if (dca_notNorm[childIndex] < 0.0001) else 1.0 / dca_notNorm[childIndex]]
            ...
            if (isNeutrinoLink) :
                ...
                if (passHitThreshold) :
                    if (isTrueEdge) :
                        ...
                        source_index_pos.append(iParent)
                        target_index_pos.append(iChild)
                        edge_attr_pos.append(thisEdgeAttr)
                        # and back the other way
                        source_index_pos.append(iChild) 
                        target_index_pos.append(iParent)
                        edge_attr_pos.append(thisEdgeAttr)
                        ...
                        source_index_neg.append(iParent) 
                        target_index_neg.append(iChild)
                        edge_attr_neg.append(thisEdgeAttr)
                        # and back the other way
                        source_index_neg.append(iChild)
                        target_index_neg.append(iParent)
                        edge_attr_neg.append(thisEdgeAttr)
    ...
    # turn into data
    data_pos = Data(x=event_node_features, edge_index=event_edge_index_pos, y=event_node_truth, edge_attr=edge_attr_pos)
    data_neg = Data(x=event_node_features, edge_index=event_edge_index_neg, y=event_node_truth, edge_attr=edge_attr_neg)
    data_FC = Data(x=event_node_features, edge_index=event_edge_index_FC, y=event_node_truth, edge_attr=edge_attr_FC)
