...
def InferenceGraphBuilder(eventDict, modeDict) :
    ...
    if (nGraphNodes == 1) :
        ...
        return Data(x=event_node_features, edge_index=event_edge_index_FC, y=event_node_truth, edge_attr=edge_weight_FC), np.empty(0)
    ...
    for iSourceParticle in range(nGraphNodes) :
        ...
        for iTargetParticle in range((iSourceParticle + 1), nGraphNodes) :
            ...
            if isNeutrinoLink :
                ...
                target_index_FC.append(iChild)
                # Add the edge weight - smaller DCA is more important
                edge_weight_FC.append([0.0 if (dca_notNorm[childIndex] < 0.0001) else 1.0 / dca_notNorm[childIndex]])
            else :
                # Calculate opening angle - do need to worry about normalisation
                sourceDirection = np.array(showerDir_notNorm[sourcePFPIndex])
                targetDirection = np.array(showerDir_notNorm[targetPFPIndex])
    ...
    # turn into data
    data_FC = Data(x=event_node_features, edge_index=event_edge_index_FC, edge_attr=edge_weight_FC)
